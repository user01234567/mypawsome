...
async def auth_callback(request: Request):
    ...
    if "id_token" in token:
        try:
            ...
            if not user_info:
                raise HTTPException(status_code=500, detail="No userinfo in token fallback")
    else:
    ...
    user = {
        ...
        "username": user_info.get("name") or user_info.get("preferred_username") or user_info.get("username"),
...
async def create_tierlist(
    ...
    if not isinstance(tier_defs, list) or len(tier_defs) == 0:
        raise HTTPException(status_code=400, detail="Tierlist must have at least one tier definition.")
    ...
    q = select(tiers).where(tiers.c.tierlist_id == new_tierlist_id).order_by(tiers.c.position)
    created_tiers = await database.fetch_all(q)
    return {"tierlist_id": new_tierlist_id, "name": name, "tiers": [dict(row) for row in created_tiers]}

@app.get("/tierlists")
async def list_tierlists():
    rows = await database.fetch_all(select(tierlists))
    return [{"id": row["id"], "name": row["name"], "creator_id": row["creator_id"]} for row in rows]

@app.get("/tierlists/{tierlist_id}")
async def get_tierlist(tierlist_id: int = FPath(..., description="ID of the tierlist to fetch")):
    tl = await database.fetch_one(select(tierlists).where(tierlists.c.id == tierlist_id))
    if not tl:
...
async def add_item(
    ...
    if tier_id is not None:
        if not await database.fetch_one(
            select(tiers.c.id).where((tiers.c.id == tier_id) & (tiers.c.tierlist_id == tierlist_id))
        ):
            raise HTTPException(status_code=400, detail="tier_id is invalid for this tierlist.")
...
async def cast_vote(
    ...
    row_item = await database.fetch_one(select(items.c.tierlist_id).where(items.c.id == item_id))
    if not row_item:
        raise HTTPException(status_code=404, detail="Item not found.")
    ...
    row_tier = await database.fetch_one(
        select(tiers.c.id).where((tiers.c.id == tier_id) & (tiers.c.tierlist_id == tierlist_of_item))
    )
    if not row_tier:
    ...
    existing = await database.fetch_one(
        select(votes.c.id).where((votes.c.user_id == uid) & (votes.c.item_id == item_id))
    )
...
async def get_tierlist_votes(
    ...
    tl = await database.fetch_one(select(tierlists.c.id).where(tierlists.c.id == tierlist_id))
    if not tl:
        raise HTTPException(status_code=404, detail="Tierlist not found.")

    # 2️⃣ Fetch all items in that tierlist
    item_rows = await database.fetch_all(select(items).where(items.c.tierlist_id == tierlist_id))
